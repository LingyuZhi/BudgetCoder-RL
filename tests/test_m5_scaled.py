...
def test_historical_freeze_files_untouched():
    ...
    assert sha256_file(REPO_ROOT / "configs/experiments/stage1_m3c_freeze.json") == EXPECTED_M3C_FREEZE_SHA256
    assert sha256_file(REPO_ROOT / "configs/experiments/stage1_m5_main.json") == EXPECTED_M5_MAIN_SHA256
...
def test_written_scaled_manifest_and_contract():
    ...
    freeze = load_json(REPO_ROOT / "configs/experiments/stage1_m3c_freeze.json")
    rebuilt = build_scaled_contract(
        freeze=freeze,
        freeze_path=REPO_ROOT / "configs/experiments/stage1_m3c_freeze.json",
        candidate=payload,
        candidate_path=path,
        envelope_path=REPO_ROOT
        / "configs/experiments/stage1_canonical_execution_envelope.json",
        project_commit=None,
    )
    assert scaled_contract_errors(rebuilt) == []
    overlay = load_json(default_preflight_path(REPO_ROOT))
