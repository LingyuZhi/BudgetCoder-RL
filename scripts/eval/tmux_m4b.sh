#!/usr/bin/env bash
# Launch M4B GPU one-step GRPO optimizer smoke inside tmux on n30158.
set -uo pipefail
...
LOG_DIR="outputs/experiments/E003"
mkdir -p "$LOG_DIR"
exec > >(tee -a "$LOG_DIR/pipeline.log") 2>&1
python scripts/eval/run_m4b_gpu.py --experiment-id E003 --skip-gpu-pick
echo "m4b_exit=$?"
exec bash
