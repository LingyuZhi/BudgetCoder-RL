#!/usr/bin/env bash
# Launch M3C GPU pipeline inside tmux on n30158.
set -uo pipefail
...
export BCRL_M3C_STAGE="${BCRL_M3C_STAGE:-gpu}"
export CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0}"
export TOKENIZERS_PARALLELISM=true
cd "$(dirname "$0")/../.."
echo "host=$(hostname) gpu=$CUDA_VISIBLE_DEVICES pwd=$(pwd) python=$(which python) stage=$BCRL_M3C_STAGE"
nvidia-smi --query-gpu=index,memory.used,utilization.gpu --format=csv,noheader || true
LOG_DIR="outputs/experiments/E006"
mkdir -p "$LOG_DIR"
exec > >(tee -a "$LOG_DIR/pipeline.log") 2>&1
bash scripts/eval/run_m3c_pipeline.sh
echo "pipeline_exit=$?"
exec bash
