#!/usr/bin/env python
"""E013 2-GPU 20480 envelope headroom: 2 long prompts, 1 GRPO update.

Does not edit E012/E011/stage1_m5_main.json. Does not run the 32-step main.

Usage (compute node, conda env ``verl``):

    python scripts/eval/run_e013_envelope.py
"""
...
from budget_coder_rl.eval.e013 import (  # noqa: E402
    CHOSEN_ENVELOPE,
    EXPERIMENT_ID,
    ...
    default_e012_runtime_path,
    default_e013_output_dir,
    default_stress_checkpoint_dir,
    e012_untouched_errors,
    is_login_host,
    packing_smokes,
    peak_gpu_memory_mib,
    ranked_from_e012_audit,
    ready_payload,
    resource_lifecycle,
...
def _assert_compute_node() -> str:
    ...
    if is_login_host(host):
        raise SystemExit(f"HARD FAIL: do not run E013 GPU on login node ({host})")
    try:
        import subprocess
...
def main(argv: list[str] | None = None) -> int:
    ...
    output_dir = (
        ...
        else default_e013_output_dir(repo_root)
    )
    if output_dir.resolve() == (repo_root / "outputs" / "experiments" / "E012").resolve():
        print("HARD FAIL: refusing to write into E012 artifact directory", file=sys.stderr)
        return 1
    ...
    if sha256_file(main_path) != EXPECTED_MAIN_SHA256:
        ...
        return 1
    e012_errors = e012_untouched_errors(repo_root)
    if e012_errors:
        print(f"HARD FAIL: E012 mutated {e012_errors}", file=sys.stderr)
        return 1
    ...
    try:
        ranked = ranked_from_e012_audit(repo_root)
    ...
    if len(selected) != STRESS_UNIQUE_TASKS or REQUIRED_TASK not in selected:
        print(f"HARD FAIL: selection invalid {selected}", file=sys.stderr)
        return 1
    ...
    train_parquet = output_dir / "train_e013_stress.parquet"
    ...
    provenance["phase"] = "envelope_headroom_stress"
    provenance["instance_ids"] = selected
    provenance["selection"] = {
        "universe": "E012 capacity_audit.json ranked_top (read-only)",
        "filter": "2 longest prompts; required dask__dask-10042",
        "n_unique_tasks": len(selected),
        "n_steps": STRESS_STEPS,
        "train_batch_size_stress": STRESS_TRAIN_BATCH_SIZE,
    ...
    provenance["e012_overlay_sha256"] = EXPECTED_E012_OVERLAY_SHA256
    provenance["artifacts"] = artifact_hashes(
        {
            ...
            "e012_overlay": default_e012_runtime_path(repo_root),
    ...
    try:
        ...
        apply_m5_train_config(
            ...
            n_tasks=int(STRESS_TRAIN_BATCH_SIZE),
            n_gpus=n_gpus,
            ppo_max_token_len_per_gpu=ppo_max,
            total_training_steps=STRESS_STEPS,
            experiment_name="E013-envelope-headroom",
        ...
        if int(config.data.train_batch_size) != STRESS_TRAIN_BATCH_SIZE:
            raise SystemExit("HARD FAIL: stress train_batch_size mismatch")
    ...
    mem_ok, mem_reasons = memory_healthy(oom=oom, peak_mib=gpu_peak.get("peak_mib"))
    research_unmodified = sha256_file(main_path) == EXPECTED_MAIN_SHA256 and not knob_errors
    e012_still = not e012_untouched_errors(repo_root)
    reasons: list[str] = []
    if not packing.get("covers"):
    ...
    if not e012_still:
        reasons.append("e012_overlay_mutated")
    ...
    evidence = {
        ...
        "READY_TO_FREEZE_CANONICAL": ready,
    ...
    write_json(
        output_dir / "READY_TO_FREEZE_CANONICAL.json",
        ready_payload(ready=ready, reasons=reasons, extra=evidence),
    )
    (output_dir / "SUMMARY.md").write_text(
        build_headroom_summary(evidence=evidence),
        encoding="utf-8",
    )
    if ready:
        frozen = write_canonical_envelope(
            repo_root,
            overlay_sha256=str(runtime.get("overlay_sha256") or ""),
            evidence=evidence,
        )
        write_json(output_dir / "canonical_execution_envelope.json", frozen["payload"])
        evidence["canonical_envelope_sha256"] = frozen["sha256"]
        write_json(output_dir / "run_status.json", evidence)
    print(
        json.dumps(
            {
