#!/usr/bin/env bash
# M7D trainer-path forensic: start | status | attach | logs | cpu
# Diagnostic only. Does not train, modify E017/E018/M7C, or enter intervention.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
SESSION="${M7D_SESSION_NAME:-M7D}"
COMPUTE_HOST="${BCRL_COMPUTE_HOST:-n30158}"
LOG_DIR="${ROOT}/outputs/experiments/M7D"
LOG_FILE="${LOG_DIR}/pipeline.log"
SESSION_INFO="${LOG_DIR}/session_info.json"
CMD="${1:-status}"
TRACE_DIR="${BCRL_DATA_ROOT:-$HOME/my_data/budget-coder-rl}/trajectories/m7d/M7D"
...
maybe_forward() {
  ...
  if is_login_host; then
    exec ssh -o BatchMode=yes -o StrictHostKeyChecking=accept-new "${COMPUTE_HOST}" \
      "cd $(printf '%q' "${ROOT}") && \
       export M7D_ON_COMPUTE=1 BCRL_DATA_ROOT=$(printf '%q' "${BCRL_DATA_ROOT:-$HOME/my_data/budget-coder-rl}") && \
       bash scripts/eval/m7d_session.sh $(printf '%q' "${CMD}")"
  fi
}
...
inner_start() {
  ...
  python -m pytest tests/test_m7d.py tests/test_m7c.py tests/test_m7a.py -q
  echo "m7d_pytest_exit=$?"

  python scripts/eval/analyze_m7d_first_request.py --output-dir "${LOG_DIR}"
  local audit_rc=$?
  echo "m7d_audit_exit=${audit_rc}"
  python - <<PY
import json
from pathlib import Path
gate = json.loads(Path("${LOG_DIR}/audit_gate.json").read_text())
if not gate.get("allow_gpu"):
    raise SystemExit(f"HARD FAIL: first-request confound {gate}")
print("allow_gpu", gate.get("allow_gpu"))
print("aliasing", gate.get("aliasing"))
PY
  ...
  python scripts/eval/run_m7d_cells.py --max-hours 8
  local replay_rc=$?
  echo "m7d_replay_exit=${replay_rc}"
  ...
  python scripts/eval/analyze_m7d_replay.py \
    --episodes "${SCORED}" \
    --output-dir "${LOG_DIR}"
...
cmd_cpu() {
  ...
  cd "${ROOT}"
  mkdir -p "${LOG_DIR}"
  python -m pytest tests/test_m7d.py -q
  python scripts/eval/analyze_m7d_first_request.py --output-dir "${LOG_DIR}"
}
...
cmd_start() {
  ...
  tmux new-session -d -s "${SESSION}" -c "${ROOT}" \
    env \
      ...
      bash -lc "cd $(printf '%q' "${ROOT}") && exec > >(tee -a $(printf '%q' "${LOG_FILE}")) 2>&1 && bash scripts/eval/m7d_session.sh _inner"
  sleep 2
  if ! tmux has-session -t "${SESSION}" 2>/dev/null; then
  ...
  echo "M7D started: tmux session=${SESSION} log=${LOG_FILE}"
}
