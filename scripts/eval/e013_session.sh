#!/usr/bin/env bash
# E013 SSH-independent 20480 envelope headroom: start | status | attach | logs
# WANDB_API_KEY must be in the environment for `start`. Never echo it.
# Does not touch tmux E012 / E011.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
SESSION="${E013_SESSION_NAME:-E013}"
COMPUTE_HOST="${BCRL_COMPUTE_HOST:-n30158}"
LOG_DIR="${ROOT}/outputs/experiments/E013"
...
maybe_forward() {
  ...
  if is_login_host; then
    ...
    exec ssh -o BatchMode=yes -o StrictHostKeyChecking=accept-new "${COMPUTE_HOST}" \
      "cd $(printf '%q' "${ROOT}") && \
       export E013_ON_COMPUTE=1 BCRL_DATA_ROOT=$(printf '%q' "${BCRL_DATA_ROOT:-$HOME/my_data/budget-coder-rl}") \
       ${quoted_key} && bash scripts/eval/e013_session.sh $(printf '%q' "${CMD}")"
  fi
}
...
inner_start() {
  ...
  python scripts/eval/run_e013_envelope.py --experiment-id E013
  local rc=$?
  echo "e013_exit=${rc}"
...
cmd_start() {
  ...
  tmux new-session -d -s "${SESSION}" -c "${ROOT}" \
    env \
      ...
      bash -lc "cd $(printf '%q' "${ROOT}") && exec > >(tee -a $(printf '%q' "${LOG_FILE}")) 2>&1 && bash scripts/eval/e013_session.sh _inner"
  sleep 2
  if ! tmux has-session -t "${SESSION}" 2>/dev/null; then
  ...
  echo "E013 started: tmux session=${SESSION} pane_pid=${pane_pid} log=${LOG_FILE}"
}
...
cmd_status() {
  ...
  if [[ -f "${LOG_DIR}/run_status.json" ]]; then
    python3 -c "import json; p=json.load(open('${LOG_DIR}/run_status.json')); print('run_status', p.get('status'), p.get('READY_TO_FREEZE_CANONICAL'), p.get('stop_reason'), 'steps', p.get('n_steps_completed'), 'peak', p.get('gpu_peak_mib'))"
  else
