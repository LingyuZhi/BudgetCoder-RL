#!/usr/bin/env bash
# E016 SSH-independent scaled-M5 2-step preflight: start | status | attach | logs
# WANDB_API_KEY must be in the environment for `start`. Never echo it.
# Does not start the 275-step main run. Does not touch tmux E014 / E015.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
SESSION="${E016_SESSION_NAME:-E016}"
COMPUTE_HOST="${BCRL_COMPUTE_HOST:-n30158}"
LOG_DIR="${ROOT}/outputs/experiments/E016"
...
maybe_forward() {
  ...
  if is_login_host; then
    ...
    exec ssh -o BatchMode=yes -o StrictHostKeyChecking=accept-new "${COMPUTE_HOST}" \
      "cd $(printf '%q' "${ROOT}") && \
       export E016_ON_COMPUTE=1 BCRL_DATA_ROOT=$(printf '%q' "${BCRL_DATA_ROOT:-$HOME/my_data/budget-coder-rl}") \
       ${quoted_key} && bash scripts/eval/e016_session.sh $(printf '%q' "${CMD}")"
  fi
}
...
inner_start() {
  ...
  python scripts/eval/e011_gpu_sampler.py --output "${LOG_DIR}/gpu_sampler.jsonl" --interval 15 &
  local sampler_pid=$!
  echo "gpu_sampler_pid=${sampler_pid}"
  trap 'kill '"${sampler_pid}"' 2>/dev/null || true' EXIT
  python scripts/eval/run_e016_preflight.py --experiment-id E016
  local rc=$?
  echo "e016_exit=${rc}"
...
cmd_start() {
  ...
  tmux new-session -d -s "${SESSION}" -c "${ROOT}" \
    env \
      ...
      bash -lc "cd $(printf '%q' "${ROOT}") && exec > >(tee -a $(printf '%q' "${LOG_FILE}")) 2>&1 && bash scripts/eval/e016_session.sh _inner"
  sleep 2
  if ! tmux has-session -t "${SESSION}" 2>/dev/null; then
  ...
  echo "E016 started: tmux session=${SESSION} pane_pid=${pane_pid} log=${LOG_FILE}"
}
...
cmd_status() {
  ...
  if [[ -f "${LOG_DIR}/run_status.json" ]]; then
    python3 -c "import json; p=json.load(open('${LOG_DIR}/run_status.json')); print('run_status', p.get('status'), p.get('READY_FOR_SCALED_M5_MAIN'), p.get('stop_reason'), 'steps', p.get('n_steps_completed'), 'do_not_start_main', p.get('do_not_start_main'))"
  else
