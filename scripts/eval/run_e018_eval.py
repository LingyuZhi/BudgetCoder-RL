#!/usr/bin/env python
"""E018 scaled-M6 frozen held-out eval. Eval only. No GRPO.

Base and Scaled RL use separate Ray sessions. Candidate is E017
global_step_275 only. Treatment-integrity failure stops the RL matrix.
"""
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--phase", choices=("smoke", "base", "rl", "all"), required=True)
...
def _assert_compute_node() -> str:
    ...
    if is_login_host(host):
        raise SystemExit(f"HARD FAIL: do not run E018 GPU on login node ({host})")
    try:
        import subprocess
...
def init_e018_rl_manager(config, *, checkpoint_actor_dir: str, output_dir: Path):
    ...
    from budget_coder_rl.train.e018_actor import E018ActorWorker
    from budget_coder_rl.train.e018_vllm_server import register_e018_vllm_replica
    from budget_coder_rl.train.m4b_trainer import _unwrap_snapshot
    from gpu_runtime import query_vllm_lora_ids
...
def main(argv: list[str] | None = None) -> int:
    ...
    if int(args.n_gpus) != N_GPUS:
        print(f"HARD FAIL: E018 requires n_gpus={N_GPUS}", file=sys.stderr)
        return 1
    ...
    if M3C_AGENT_LOOP_CONFIG_RELPATH not in str(agent_loop_config):
        print("HARD FAIL: E018 must use the M3C AgentLoop YAML", file=sys.stderr)
        return 1
