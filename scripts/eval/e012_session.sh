#!/usr/bin/env bash
# E012 SSH-independent capacity stress: start | status | attach | logs
# WANDB_API_KEY must be in the environment for `start`. Never echo it.
set -euo pipefail

ROOT="$(cd "$(dirname "$0")/../.." && pwd)"
SESSION="${E012_SESSION_NAME:-E012}"
COMPUTE_HOST="${BCRL_COMPUTE_HOST:-n30158}"
LOG_DIR="${ROOT}/outputs/experiments/E012"
...
maybe_forward() {
  ...
  if is_login_host; then
    ...
    exec ssh -o BatchMode=yes -o StrictHostKeyChecking=accept-new "${COMPUTE_HOST}" \
      "cd $(printf '%q' "${ROOT}") && \
       export E012_ON_COMPUTE=1 BCRL_DATA_ROOT=$(printf '%q' "${BCRL_DATA_ROOT:-$HOME/my_data/budget-coder-rl}") \
       ${quoted_key} && bash scripts/eval/e012_session.sh $(printf '%q' "${CMD}")"
  fi
}
...
inner_start() {
  ...
  python scripts/eval/run_e012_capacity.py --experiment-id E012
  local rc=$?
  echo "e012_exit=${rc}"
...
cmd_start() {
  ...
  tmux new-session -d -s "${SESSION}" -c "${ROOT}" \
    env \
      ...
      bash -lc "cd $(printf '%q' "${ROOT}") && exec > >(tee -a $(printf '%q' "${LOG_FILE}")) 2>&1 && bash scripts/eval/e012_session.sh _inner"
  sleep 2
  if ! tmux has-session -t "${SESSION}" 2>/dev/null; then
  ...
  echo "E012 started: tmux session=${SESSION} pane_pid=${pane_pid} log=${LOG_FILE}"
}
...
cmd_status() {
  ...
  if [[ -f "${LOG_DIR}/run_status.json" ]]; then
    python3 -c "import json; p=json.load(open('${LOG_DIR}/run_status.json')); print('run_status', p.get('status'), p.get('READY_FOR_E012_MAIN'), p.get('stop_reason'), 'steps', p.get('n_steps_completed'))"
  else
