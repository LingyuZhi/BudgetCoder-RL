#!/usr/bin/env python
"""Canonical GRPO + LoRA training for Qwen3-4B repository exploration.

Uses RepoExplorationAgentLoop, localization reward, and the scaled training
contract (2193 unique / 2200 rows / 275 steps / G=4). Scientific knobs are
frozen; this entrypoint does not retune them.

Usage (compute node, pinned conda env)::
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--config",
        type=Path,
        default=REPO_ROOT / "configs/training/grpo_qwen3_4b.json",
        help="canonical training config (default: configs/training/grpo_qwen3_4b.json)",
    )
    ...
    parser.add_argument(
        "--dry-run",
        action="store_true",
        help="Load config and verify frozen contracts; do not launch Ray/PPO.",
    )
    parser.add_argument(
        "--allow-dirty",
        action="store_true",
        help="Allow a dirty worktree. Default refuses dirty.",
    )
    parser.add_argument(
        "--allow-resume",
        action="store_true",
        help="Resume the same checkpoint dir after an infrastructure interrupt.",
    )
...
def _assert_compute_node() -> str:
    ...
    if is_login_host(host):
        raise SystemExit(f"HARD FAIL: do not run GRPO GPU on login node ({host})")
    try:
        import subprocess
...
def main(argv: list[str] | None = None) -> int:
    ...
    if not canonical_path.is_file():
        print(f"HARD FAIL: missing training config {canonical_path}", file=sys.stderr)
        return 1
    canonical = load_json(canonical_path)
    if args.dry_run:
        print(f"config: {canonical_path}")
        print(f"model: {canonical.get('model')}")
        print(f"agent_loop: {canonical.get('agent_loop_config')}")
        trainer = (canonical.get("trainer") or {})
        algo = (canonical.get("algorithm") or {})
        print(f"steps: {trainer.get('total_training_steps')} G={algo.get('rollout_n')}")
        print(f"scaled contract sha256 ok: {EXPECTED_CONTRACT_SHA256}")
        print("dry-run: contracts loaded; not launching Ray/PPO")
        return 0
    ...
    verl_source = default_verl_source(repo_root)
    verl_info = ensure_isolated_verl_checkout(
        isolated_root=isolated_root,
        source_git=verl_source,
        create=True,
    )
    ...
    lifecycle.pop("compute_host_hint", None)

    checkpoint_template = str(
        ((canonical.get("checkpoint") or {}).get("directory_template"))
        or f"$BCRL_DATA_ROOT/{CHECKPOINT_RELPATH}"
    )
    checkpoint_root = Path(
        os.path.expandvars(checkpoint_template.replace("$BCRL_DATA_ROOT", str(data_root)))
    )
    ckpt_path_errors = [
        f"refusing checkpoint path containing {marker}"
        for marker in FORBIDDEN_CHECKPOINT_MARKERS
        if marker in str(checkpoint_root)
    ]
    if ckpt_path_errors:
    ...
    provenance["artifacts"] = artifact_hashes(
        {
            ...
            "canonical_envelope": repo_root / "configs/historical/stage1_canonical_execution_envelope.json",
            "oracle": oracle_path,
            "agent_loop_config": agent_loop_config,
    ...
    from budget_coder_rl.train.grpo_trainer import M5TaskRunner
    ...
    try:
        ...
        if str(checkpoint_root) not in str(config.trainer.default_local_dir) and checkpoint_root.name not in str(
            config.trainer.default_local_dir
        ):
            raise SystemExit("HARD FAIL: default_local_dir is not the configured checkpoint dir")
